<template>
    <view class="image-upload-wrapper">
        ...
        <view class="uploaded-list">
            <view v-if="currentFileList.length > 0" class="uploaded-list">
                <view class="uploaded-item" v-for="(item, index) in currentFileList" :key="index">
                    <image :src="item.url" mode="aspectFill" class="uploaded-img"
                        @touchstart="handleTouchStart($event, index)" @touchmove="handleTouchMove($event)"
                        @touchend="handleTouchEnd($event, index)"></image>
                </view>
            </view>
            <!-- 上传区域 -->
        ...
        <!-- 删除确认弹窗 -->
        <u-modal :show="deleteModalVisible" title="提示" content="确定要删除这张图片吗？" confirm-text="删除" cancel-text="取消"
            ...
            return new Promise((resolve, reject) => {
                ...
                uni.chooseImage({
                    ...
                    success: (res) => {
                        ...
                        if (concurrent) {
                            Promise.all(
                                files.map(file => this.uploadFileToOss(file, progressCallback, this.ossPath))
                            ).then(results => {
                            ...
                            const uploadNext = (index) => {
                                ...
                                this.uploadFileToOss(files[index], progressCallback, this.ossPath)
                                    .then(result => {
                                        results.push(result)
                                        uploadNext(index + 1)
                                    })
