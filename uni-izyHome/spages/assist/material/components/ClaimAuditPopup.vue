<template>
  ...
  >
    <view class="audit-popup">
      <view class="audit-title">审核物资申领</view>

      <view class="audit-info">
        <text class="info-line">商品：{{ claim.goodsTitle || '--' }}</text>
        <text class="info-line">申领人：{{ claim.contactName || '--' }}</text>
        <text class="info-line">数量：{{ claim.claimCount || 0 }}</text>
        <text class="info-line">原因：{{ claim.claimReason || '--' }}</text>
      </view>
...
<script>
import { audit2 } from '@/spages/api/material';

export default {
  name: 'ClaimAuditPopup',
  props: {
    show: { type: Boolean, default: false },
    claim: { type: Object, default: () => ({}) },
  },
  watch: {
    show(val) { if (val) this.auditRemark = ''; },
  },
  data() {
    return { auditRemark: '', loading: false };
  },
  methods: {
    async handleAudit(approved) {
      this.loading = true;
      try {
        const res = await audit2({
          claimId: this.claim.claimId,
          approved,
          auditRemark: this.auditRemark,
        });
        if (res.code === '00000') {
          uni.showToast({ title: approved ? '审核通过' : '已驳回', icon: 'none' });
          this.$emit('done');
          this.handleClose();
        } else {
...
.audit-popup {
  ...
  .audit-info {
    ...
    .info-line {
      font-size: 26rpx; color: #333; display: block; margin-bottom: 8rpx;
  ...
  .audit-form {
    margin-bottom: 28rpx;
    .form-label { font-size: 26rpx; color: #333; margin-bottom: 10rpx; display: block; }
    .audit-textarea {
  ...
  .audit-footer {
    ...
    .btn-reject { background: #fde8e8; color: #ef4444; }
