    ...
    <view class="create-container">
        ...
        <view class="form-wrapper">
            ...
            <view class="form-card">
                <u-input type="text" :border="false" v-model="form.title" placeholder="请输入活动标题 (最多20字)" :maxlength="20"
                    class="title-input" />
                <u-line color="#edf0f2"></u-line>
                <u-textarea :border="false" v-model="form.content" placeholder="请输入活动内容，可以让大家了解你的活动 (最多500字)"
                    :maxlength="500" class="content-textarea" :height="100"></u-textarea>
            </view>
            ...
            <view class="form-card">
                ...
                <view class="picker-selector-item" @click="chooseLocation">
                    <text class="picker-value" :class="{ 'placeholder-style': !form.location }">
                        {{ form.location || '请选择' }}
                    </text>
            ...
            <view class="form-card">
                ...
                <view class="time-select-row">
                    ...
                    <view class="time-picker-box" @click="openTimePicker('start')">
                        <text class="time-text" :class="{ 'placeholder-style': !form.startTime }">
                ...
                <view class="time-select-row">
                    ...
                    <view class="time-picker-box" @click="openTimePicker('end')">
                        <text class="time-text" :class="{ 'placeholder-style': !form.endTime }">
                            {{ form.endTime || '请选择时间' }}
                        </text>
                    </view>
                </view>
            ...
            <view class="form-card">
                ...
                <view class="field-row" @click="openCommunitySelector">
                    ...
                    <view class="field-value">
                        ...
                        <u-icon name="arrow-right" color="#b2b2b2" size="14" style="margin-left: 12rpx;"></u-icon>
                    </view>
                </view>
                ...
                <view class="field-row">
                    ...
                    <input type="number" v-model="form.maxLimit" placeholder="不限人数" class="limit-input" />
                </view>
                <u-line color="#f5f7fa"></u-line>
                ...
                <view class="field-row">
                    ...
                    <u-switch v-model="form.collectPhone" activeColor="#07c160" size="20"></u-switch>
                </view>
            </view>
        </view>
        ...
        <view class="footer-buttons">
            ...
            <button class="btn-submit" :class="{ 'btn-submit-active': isFormValid }" @click="handleSubmit">
                立即发布
            </button>
        </view>

        <!-- uView 2.x 时间选择器 minDate绑定动态变量pickerMinTime -->
        <u-datetime-picker :show="timePickerShow" v-model="tempTime" mode="datetime" :minDate="pickerMinTime"
            :formatter="formatter" @confirm="confirmTime" @cancel="timePickerShow = false"></u-datetime-picker>

        <!-- 社区选择组件 -->
        <CommunitySelector :show.sync="showCommunitySelector" title="请选择活动社区" mode="select"
            @confirm="handleCommunityChange" nocache />
    </view>
</template>
...
export default {
    ...
    computed: {
        // 表单基础非空判定
        isFormValid() {
            return this.form.title && this.form.content && this.form.location && this.form.startTime && this.form.endTime;
        }
    },
    onLoad(options) {
        ...
        if (options) {
            ...
            if (options.content) this.form.content = decodeURIComponent(options.content);
    ...
    methods: {
        ...
        confirmTime(e) {
            ...
            if (this.activeTimeType === 'start') {
                ...
                if (this.form.endTime && new Date(this.form.endTime).getTime() < selectTs) {
                    this.form.endTime = '';
                }
            } else {
