...
MinCache.install = function (Vue, {
  ...
  Vue.prototype.$map = new MinCache(timeout);
};

export default MinCache;
// 定义一个基于Map和uni.storage的缓存工具类

/**使用方法 main.js
 * import MinCache from './MinCache'
 *  注册缓存器 Vue.use(MinCache)
 * 或者设置默认缓存时间  Vue.use(MinCache, {timeout: 600})
 * 		// 'name'不是以下划线开头的表示会缓存到Map中，在程序生命周期内有并且在有效时间内有效
      this.$map.set('name', 'MinCache')

      // 过期时间设置为0表示不会过期
      // 注意：'test'并不是以下划线命名表示在程序生命周期永久缓存
      this.$map.set('test', 'testdemo', 0)
 * 检查是否含有
 * this.$map.has("_imgURL")
 // 过期时间设置为0表示不会过期
        // 注意：'_imgURL'是以下划线命名表示永久缓存到Storage
        this.$map.set("_imgURL", data, 0);
 * 
 */